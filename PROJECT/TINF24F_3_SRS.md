...
| **Postcondition on Success**| A new AAS has been successfully generated, in which all supported data selected by the user is mapped from the model file as submodel elements. The created AAS is stored on the AAS-Server. Two submodels were created, "HandoverDocumentation" with the original File attached and "TechnicalData" with the extracted datapoints. |
